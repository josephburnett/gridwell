...
| `apps/desktop/e2e/federation.spec.ts` | 2026-09-21, CI run 35618756780 at `27192fa5`: "right-drag deep-copies a solid well across nodes" failed at `the deep copy landed in home: Received undefined`, and the retry passed | the client's idle signal counted only the writes whose call site remembered to count them. `postTileMutate` bumped a counter; `commitRightClone`'s CloneTile and `commitDrop`'s PlaceTile posted straight to the dispatcher and bumped nothing, so `__gridwellTest.idle()` read true for the whole round trip of a cross-node deep copy and the spec's next line asked the server oracle for a tile the node was still making. The left-drag test above it never flaked because a cross-node link goes through `createTile`, which is a `postTileMutate` | the dispatcher owns the tally: `post` and `do` count every write in flight (`client/inflight.Writes`), so a gesture cannot post a write a spec does not wait for |
