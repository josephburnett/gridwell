...
| **status_detail** | The owning plugin's one word about a tile's state ("unread", "done"), drawn muted after the name (`tileface.BannerRuns`). A note on a name, never a second name, and nothing outside the plugin can derive it. |
