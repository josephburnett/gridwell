...
type sink struct {
	cpv1.ContentProvider_ServeContentServer
	chunks []*cpv1.ServeContentChunk
}

func (s *sink) Send(c *cpv1.ServeContentChunk) error { s.chunks = append(s.chunks, c); return nil }
...
func TestListsWeeksThenTodosAndRefreshesOnAWindow(t *testing.T) {
	...
	info, _ := p.Info(ctx, &cpv1.InfoRequest{})
	if info.Kind != Kind || info.RootContext != todos.RootContext {
		t.Fatalf("info = %v", info)
	}
	root, err := p.List(ctx, &cpv1.ListRequest{Context: todos.RootContext})
	if err != nil {
		t.Fatal(err)
	}
	...
	wk, err := p.List(ctx, &cpv1.ListRequest{Context: "week:2026-08-17"})
	if err != nil {
		t.Fatal(err)
	}
	if src.calls != 2 {
	...
	wk, _ = p.List(ctx, &cpv1.ListRequest{Context: "week:2026-08-17"})
	...
	root, _ = p.List(ctx, &cpv1.ListRequest{Context: todos.RootContext})
...
func TestServeContentAndProbe(t *testing.T) {
	...
	ctx := context.Background()
	if _, err := p.List(ctx, &cpv1.ListRequest{Context: todos.RootContext}); err != nil {
		t.Fatal(err)
	}
	s := &sink{}
	if err := p.ServeContent(&cpv1.ServeContentRequest{Key: "todo:1"}, s); err != nil {
		t.Fatal(err)
	}
	...
	s = &sink{}
	_ = p.ServeContent(&cpv1.ServeContentRequest{Key: "todo:99"}, s)
	if c := s.chunks[0]; c.Status != 404 || !strings.Contains(string(c.Data), "todo:99") {
		t.Errorf("unknown todo = %d %s", c.Status, c.Data)
	}
	s = &sink{}
	_ = p.ServeContent(&cpv1.ServeContentRequest{Key: "todo:1", Subpath: "x"}, s)
	if s.chunks[0].Status != 404 {
		t.Error("a subpath must 404")
	}
	probe := func(key string) cpv1.ProbeResponse_Presence {
		r, _ := p.Probe(ctx, &cpv1.ProbeRequest{Key: key})
		return r.Presence
	}
	if probe("todo:1") != cpv1.ProbeResponse_PRESENCE_PRESENT || probe("todo:99") != cpv1.ProbeResponse_PRESENCE_UNSPECIFIED ||
		probe("week:2026-08-17") != cpv1.ProbeResponse_PRESENCE_PRESENT || probe("week:2026-01-05") != cpv1.ProbeResponse_PRESENCE_UNSPECIFIED ||
		probe("junk") != cpv1.ProbeResponse_PRESENCE_GONE {
		t.Error("probe verdicts: known PRESENT, unknown UNSPECIFIED (never GONE), malformed GONE")
	}
	res, _ := p.Search(ctx, &cpv1.SearchRequest{Query: "REVIEW"})
...
func TestNoTokenSurfacesAsAVerdict(t *testing.T) {
	...
	if _, err := p.Info(context.Background(), &cpv1.InfoRequest{}); err != nil {
		t.Error("Info must still answer so the plugin is listed")
	}
	if _, err := p.List(context.Background(), &cpv1.ListRequest{Context: "bogus"}); status.Code(err) != codes.InvalidArgument {
