...
// Provider implements cpv1.ContentProviderServer.
...
func (p *Provider) List(ctx context.Context, req *cpv1.ListRequest) (*cpv1.ListResponse, error) {
	switch {
	...
	default:
		...
		return &cpv1.ListResponse{Entries: todos.WeekEntries(start, p.mem.Week(start)), Authoritative: false, SourceLabel: req.Context}, nil
	}
}
...
func (p *Provider) ReadContent(req *cpv1.ReadContentRequest, stream cpv1.ContentProvider_ReadContentServer) error {
	...
	return stream.Send(&cpv1.ContentChunk{Data: data, MediaType: "text/html; charset=utf-8"})
}

func (p *Provider) ServeContent(req *cpv1.ServeContentRequest, stream cpv1.ContentProvider_ServeContentServer) error {
	if req.Subpath != "" {
		return stream.Send(&cpv1.ServeContentChunk{Status: 404, MediaType: "text/plain", Data: []byte("not found")})
	}
	data, code := p.pageFor(req.Key)
	if data == nil {
		data = []byte("not found")
...
func (p *Provider) Search(_ context.Context, req *cpv1.SearchRequest) (*cpv1.SearchResponse, error) {
	q := strings.ToLower(strings.TrimSpace(req.Query))
	if q == "" {
		return &cpv1.SearchResponse{}, nil
	}
	limit := int(req.Limit)
	if limit <= 0 {
		limit = 50
	}
	resp := &cpv1.SearchResponse{}
	all := p.mem.All()
	for i := len(all) - 1; i >= 0 && len(resp.Results) < limit; i-- { // newest first
		...
		resp.Results = append(resp.Results, &cpv1.SearchResult{
			Entry:       entries[0],
