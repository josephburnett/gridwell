...
// Provider implements cpv1.ContentProviderServer for the process table.
...
func (p *Provider) List(_ context.Context, req *cpv1.ListRequest) (*cpv1.ListResponse, error) {
	...
	resp := &cpv1.ListResponse{Authoritative: false, SourceLabel: req.Context}
	if _, gerr := procsource.Get(p.procRoot, pid); gerr == nil {
		resp.Entries = append(resp.Entries, &cpv1.Entry{
