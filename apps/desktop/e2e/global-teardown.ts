...
// Drops the run's artifact snapshot (runtree.ts), sweeps what the run's own
// workers and runner left behind (homes.ts), and archives failure artifacts
...
// this: its leftovers go to the next run's sweep and its artifacts to that
// wipe.
const KEEP = 5;

export default function globalTeardown(): void {
  removeTree();
  sweepLeakedHomes(process.pid);
