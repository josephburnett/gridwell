...
// Issue #211: a bare LF must keep the cursor COLUMN. tmux paints TUI
// output (scroll-region feeds, row emission — how Claude Code renders)
// using LF as a keep-the-column index; xterm's convertEol option snapped
...
test('a bare LF keeps the cursor column (#211)', async ({ gw, window }) => {
  ...
  // a BARE \n, and a marker: the marker must land at column 13. tmux knows
  // nothing about the injected text and may repaint over it at any moment,
  // so each poll attempt feeds and reads in one go and asserts on that
  // snapshot (a poll-then-reread raced the repaint and read back nothing).
  await expect
    .poll(
      async () => {
