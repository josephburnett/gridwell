...
app.whenReady().then(async () => {
  ...
  const cookies = JSON.parse(put.toString('utf8')) as Array<{ name: string; value: string }>;
  const saved = cookies.find((c) => c.name === 'login');
  if (!saved || saved.value !== 'sekrit') fail(`login cookie missing from blob: ${put}`);
  console.log(`dehydrate ok: ${cookies.length} cookie(s) captured`);
