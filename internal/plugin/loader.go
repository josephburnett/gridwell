...
// third-party door — or compiled in through a Factory, as the mobile bind
// needs, since iOS forbids fork and exec. The node constructs its own home and
// transport around this call; neither is a plugin.
//
...
// by its ID: a plugin.v1 subprocess, from a binary, or a factories
// constructor, fronted by the pluginhost adapter over the node-owned store.
...
// loadPlugin materializes one plugin entry: the content process, whether a
// subprocess binary or an in-process factory, and the adapter joining it with
// the plugin's namespace of the node's store, as an ordinary
