...
// read access to at least one node referencing the blob; otherwise we return
// ErrPermissionDenied.
//
// Permission via "any node grants read" rather than "all nodes" is the
// natural rule because blobs are content-addressed and shared across nodes
// the user has no business knowing about. If the user can read one node,
...
func (s *Store) GetBlob(ctx context.Context, userID, blobID int64) ([]byte, string, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, owner_id, group_id, mode FROM nodes WHERE blob_id = ?`, blobID)
	if err != nil {
		return nil, "", err
	}
