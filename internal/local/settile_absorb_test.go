...
// The plugin-well lifecycle through the DISPATCH seam (issue #251): create
// with configure_plugin_id, adopt via the SetTile arm — the wiring the store
// tests can't see (#196's lesson: a store method the binary never routes to
// looks green forever).
func TestSetTileAdoptChildGridArm(t *testing.T) {
	p := openPlugin(t)
	root := rootGrid(t, p)
	ctx := context.Background()

	well := createTile(t, p, root, &gridwellv1.Tile{Kind: "well", X: 0, Y: 0, W: 1, H: 1, ConfigurePluginId: "sshuuid"}, nil)
	if well.ChildGridId != "" || well.ConfigurePluginId != "sshuuid" {
		t.Fatalf("create dispatch: got (child %q, configure %q), want an unconfigured plugin well",
			well.ChildGridId, well.ConfigurePluginId)
	}

	// Both birth shapes at once is refused.
	if _, err := p.CreateTile(ctx, &gridwellv1.CreateTileRequest{GridId: root,
		Tile: &gridwellv1.Tile{Kind: "well", X: 3, Y: 0, W: 1, H: 1,
			ChildGridId: "other/1", ConfigurePluginId: "sshuuid"}}); err == nil {
		t.Error("configured+configurable create must be refused")
	}

	// One op per call: adopt plus rename together is refused.
	if _, err := p.SetTile(ctx, &gridwellv1.SetTileRequest{
		TileId: well.Id, Version: well.Version, Rename: "x",
		AdoptChildGrid: &gridwellv1.AdoptChildGrid{ChildGridId: "sshuuid/c1"},
	}); err == nil {
		t.Error("adopt+rename in one call must be refused")
	}

	r, err := p.SetTile(ctx, &gridwellv1.SetTileRequest{
		TileId: well.Id, Version: well.Version,
		AdoptChildGrid: &gridwellv1.AdoptChildGrid{
			ChildGridId: "sshuuid/c1", Label: "gpu-box", ViewX: 1, ViewY: 2, ViewZoom: 0.5,
		},
	})
	if err != nil {
		t.Fatalf("adopt: %v", err)
	}
	if r.Tile.ChildGridId != "sshuuid/c1" || r.Tile.AltText != "gpu-box" {
		t.Errorf("adopted tile = (child %q, alt %q), want the instance chain and its name",
			r.Tile.ChildGridId, r.Tile.AltText)
	}
	if r.Tile.Version <= well.Version {
		t.Errorf("adopt is a user edit: version must bump (%d -> %d)", well.Version, r.Tile.Version)
	}
}
