...
// sole indicator of a DB's shape — but an unstamped pre-freeze DB takes the
// "fresh" fast-path and is stamped at the current version WITHOUT its columns
// ever being checked (tablesDDL()'s CREATE TABLE IF NOT EXISTS no-ops on the
// already-present tables, leaving their old shape intact). A column the current
// writer no longer populates (e.g. a dropped NOT NULL `blobs.size`) then lurks
// until the first insert fails its constraint — a failure the client swallows,
// so a tile silently "disappears".
//
...
// the divergent column, instead of stamp-and-proceed. The column fingerprint it
// uses is the same one the migration equivalence tests compare with.
...
// Crucially NOT cid — ALTER TABLE ADD COLUMN always appends (higher cid) while
// inline DDL places columns mid-table, so a migrated schema and a fresh one are
// equivalent with different column orders. Shared by verifySchema and the
...
func tableColumnFPs(ctx context.Context, q gridReader, table string) (map[string]colFP, error) {
	// PRAGMA can't bind params; table names come from sqlite_master / the
...
// canonicalSchema is the column shape a fresh Open materializes: the current DDL
// applied to a throwaway in-memory DB. It is the single source of truth for what
// every table's columns should be (read from the DDL, never hand-listed), so the
// guard can't drift from the schema it protects.
...
// verifySchema fails with ErrSchemaDivergence when the open DB's columns don't
// match canonicalSchema: a missing column, a differing column (type/notnull/
// default/pk), or — the case that bit us — an extra column the current writer
// never populates. The error lists every divergence so the cause is named, not
// guessed. Extra non-canonical tables are ignored (harmless; no writes target
