...
// provider is either an out-of-process binary (LoadProvider — go-plugin:
// process isolation, a separate dependency graph — the third-party door)
// or a compiled-in factory (ServeProviderInProcess: bundled binaries;
// iOS, where fork/exec does not exist); the node's providerhost adapter
...
// the host-side spawn; guest.ServeProvider (api/guest) is the binary's
// main.
package compose
