...
var plugins = []rpc.PluginInfo{
	...
	{UUID: "sshc", Label: "connections", InstanceGridID: "sshc/0"},
}

func instTiles(gridID string) map[string]rpc.Tile {
	if gridID != "sshc/0" {
		return nil
	}
	return map[string]rpc.Tile{
		"sshc/1": {ID: "sshc/1", Kind: rpc.KindWell, AltText: "rtb",
			ChildGridID: "sshc/ns1/rp1/root7", Reference: true},
	}
}
...
func TestFindPrefersTheParentGridWell(t *testing.T) {
	...
	got, kind := Find("sshc/ns1/rp1/root7", parent, plugins, instTiles)
	if kind != Well || got.ID != "loc/5" || got.AltText != "my rtb" {
		t.Fatalf("door = %+v (%v), want the parent-grid well loc/5", got, kind)
	}
}

// A picker descent has no parent-grid well; the instance well names the
// SAME connection — renamable, because it is a real row.
func TestFindFallsBackToTheInstanceWell(t *testing.T) {
	got, kind := Find("sshc/ns1/rp1/root7", nil, plugins, instTiles)
	if kind != Well || got.AltText != "rtb" {
		t.Fatalf("door = %+v (%v), want the rtb instance well", got, kind)
	}
}
...
func TestFindResolvesRootEntries(t *testing.T) {
	got, kind := Find("loc/9", nil, plugins, instTiles)
...
func TestFindResolvesPluginRoots(t *testing.T) {
	got, kind := Find("loc/1", nil, plugins, instTiles)
...
func TestFindMissesCleanly(t *testing.T) {
	if _, kind := Find("nowhere/3", nil, plugins, instTiles); kind != None {
		t.Fatalf("unknown anchor must resolve to None, got %v", kind)
	}
	if _, kind := Find("", nil, plugins, instTiles); kind != None {
