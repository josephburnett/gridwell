...
// served page's address belongs to its plugin and an ephemeral visit to no
// row, so neither writes, and a page that never navigated has nothing new to
// write. The bridge's last reported address is the claim; the cached row's
// stands in when the bridge reported none.
...
// Page. A page view is not: its plugin owns the frozen face. Not known yet
// counts as ephemeral, because durable is a promise to write.
